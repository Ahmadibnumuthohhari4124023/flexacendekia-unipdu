...
const DataStore = {
    ...
    async getAnakByOrtuId(ortuId) {
        ...
        // Also check currentUser.anakIds in localStorage
        try {
        ...
        if (db) {
            try {
                // Strategy 2: Cek array anakIds pada dokumen ortu di Firestore
                ...
                if (ortuDoc && ortuDoc.exists && ortuDoc.data().anakIds && ortuDoc.data().anakIds.length > 0) {
                    ...
                    anakIds.forEach(id => { if (!localLinkedIds.includes(id)) localLinkedIds.push(id); });
                }
                ...
                // Strategy 3: Query koleksi users dengan filter orangTuaId / ortuId
        ...
        // Final fallback: resolve local IDs through getUserById chain
        if (localLinkedIds.length > 0) {
            const anakList = [];
            for (const id of localLinkedIds) {
                const user = await this.getUserById(id);
    ...
    async tautkanAnak(ortuUid, identifier, pin) {
        ...
        try {
            ...
            try {
                ...
                // Also save full siswa data so getUserById can find it without Firestore
